...
import { SavedSearchType } from "@/services/savedFiltersService.ts";

export interface FamilyOfficeContactSearchesProps {
  search: SavedSearchType;
  handleUseSearch: (search: SavedSearchType) => void;
  handleDeleteSearch: (id: number, query: string) => void;
}

export const FamilyOfficeContactSearches = ({
  search,
  handleUseSearch,
  handleDeleteSearch,
}: FamilyOfficeContactSearchesProps) => (
  <div key={search.id} className="p-4 hover:bg-gray-50 transition-colors">
    <div className="flex justify-between items-start">
      <div>
        <div className="flex items-center">
          <div className="font-medium text-gray-800">{search.searchQuery}</div>
      ...
      <div className="flex space-x-2">
        ...
        <Button
          ...
          onClick={() => handleDeleteSearch(search.id, search.searchQuery)}
        >
