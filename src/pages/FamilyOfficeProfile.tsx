...
import { FamilyOfficesFields } from "@/components/familyoffices/FamilyOfficesFields.tsx";
...
const FamilyOfficeProfile = () => {
  ...
  const overviewFields = [
    {
      label: "Family Office",
      value: Array.isArray(familyOffice.firm_type)
        ? familyOffice.firm_type.join(", ")
        : familyOffice.firm_type,
    },
    ...
    { label: "Wealth Creator", value: (familyOffice as any).wealth_creator },
    {
      ...
      value: (familyOffice as any).industry_wealth_origin,
    },
  ...
  const overviewFieldsContacts = [
    {
      label: "Office Phone",
      value: (familyOffice as any).office_phone,
      icon: (
        <a
          href={`tel:${(familyOffice as any).office_phone}`}
          target="_blank"
    ...
    {
      label: "Office Email",
      value: (familyOffice as any).general_email,
      icon: (
        <a
          href={`mailto:${(familyOffice as any).general_email}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
  ...
  const investmentFields = [
    {
      ...
      value: (familyOffice as any).emerging_manager_interest,
    },
    {
      label: "Emerging Markets",
      value: (familyOffice as any).emerging_markets,
    },
    {
      label: "Geographic Focus",
      value: (familyOffice as any).geographic_focus,
    },
    {
      label: "Emerging Manager Allocator",
      value: (familyOffice as any).emerging_manager_allocator,
    },
  ...
  const newsContent = Array.isArray((familyOffice as any).company_news) ? (
    <div className="flex flex-col gap-4">
      {(familyOffice as any).company_news.map((news: any, idx: number) => (
        <div
          key={idx}
          className="bg-[#F6F8FC] rounded-xl p-4 flex flex-col md:flex-row gap-4 border"
        >
          {news.publisher_logo && (
            <img
          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-gray-900 whitespace-pre-line mb-1">
              {news.content}
            </div>
          </div>
        </div>
  ...
  const transactionsContent = Array.isArray(
    (familyOffice as any).company_transactions,
  ) ? (
    <div className="flex flex-col gap-4">
      {(familyOffice as any).company_transactions.map(
        (tx: any, idx: number) => (
          <div
            key={idx}
            className="bg-[#F6F8FC] rounded-xl p-4 flex flex-col md:flex-row gap-4 border"
            ...
            <div className="flex-1">
              ...
              {tx.size && (
                <div className="text-sm text-gray-700 mb-1">
                  Deal Size: <span className="font-medium">{tx.size}</span>
                </div>
              )}
              {tx.press_release && (
                <a
      ...
      <div className="divide-y border rounded-lg overflow-hidden">
        {contacts.map((contact) => (
          <div
            ...
            className="flex flex-col md:flex-row items-start md:items-center gap-2 px-4 py-3"
          >
            <div className="flex-1">
...
const TeamMemberCard: React.FC<{
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-[#DFE4EA] p-4 flex flex-col gap-2 ${primary ? "shadow" : ""}`}
    >
